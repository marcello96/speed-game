var Coil = (function () {
    ...
    var ENEMY_TIME_BEFORE_DEATH = 100;
    ...
    var SCORE_PER_ENEMY = 1,
        SCORE_PER_TICK = 0.01;

    var ENERGY_PER_ENEMY_DEATH = -30,
        ENERGY_PER_ENEMY_ENCLOSED = 1,
        MAXIMUM_NUMBER_OF_ENEMIES = 80,
        ENERGY_PER_BOMB_ENCLOSED = -3;
            ...
            if (xhr.readyState == XMLHttpRequest.DONE) {
                ...
                if (age != null) {
                    if (age > 1 && age <= 10) {
                        difficulty = 1;
                        ENERGY_PER_BOMB_ENCLOSED = -3;
                        ENEMY_TIME_BEFORE_DEATH = 90;
                    }
                    if (age > 10 && age <= 20) {
                        difficulty = 4;
                        ENERGY_PER_BOMB_ENCLOSED = -5;
                        ENEMY_TIME_BEFORE_DEATH = 75;
                    }
                    if (age > 20) {
                        difficulty = 7;
                        ENERGY_PER_BOMB_ENCLOSED = -7;
                        ENEMY_TIME_BEFORE_DEATH = 55;
                    }
                }
            }
        }
    ...
    function stop() {
        ...
        var result = (score > 0 ? score : 0) / MAXIMUM_NUMBER_OF_ENEMIES;
        scorePanel.querySelector('p').innerHTML = result;

        var xhr = new XMLHttpRequest();
        xhr.open('POST', '/coil/end', true);
    ...
    function incrementEnemyDeaths() {
        //var multipliedOffset = 0;
        killedEnemies += 1;
        if (killedEnemies >= MAXIMUM_NUMBER_OF_ENEMIES) {
            stop();
        }
    }
    ...
    function updateEnemies() {
        ...
        while (i-- && Math.random() > 0.85) {
            ...
            if (type == ENEMY_TYPE_BOMB || (type == ENEMY_TYPE_NORMAL && createdEnemies < MAXIMUM_NUMBER_OF_ENEMIES)) {
                enemy = new Enemy();
        ...
        while (i--) {
            ...
            if (enemy.alive && enemy.time >= ENEMY_TIME_BEFORE_DEATH) {
    ...
    function renderEnemies() {
        ...
        while (i--) {
            ...
            if (enemy.type === ENEMY_TYPE_BOMB || enemy.type === ENEMY_TYPE_BOMB_MOVER) {
                ...
                if (enemy.time > ENEMY_TIME_BEFORE_DEATH * 0.65) {
    ...
    function renderHeader() {
        ...
        context.font = "10px Arial";
        context.fillStyle = "#ffffff";
        context.fillText(ENEMY_LABEL, 80, 8);

        // Enemy
        context.font = "bold 10px Arial";
        context.fillStyle = 'rgba(0,200,220, 0.8)';
        context.fillText(Math.floor(createdEnemies), 137, 8);
    ...
    function handleBombInClosure(entity) {
        ...
        adjustScore(ENERGY_PER_BOMB_ENCLOSED);
        multiplier.reset();
