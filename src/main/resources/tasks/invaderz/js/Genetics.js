class Genetics {
	...
	randomPlace(){
			return Math.random() * 0.8 * w + 0.1 * w;
	}
	...
	createPopulation(){
		...
		for(let i = 0; i < this.populationSize; i++){
			...
			this.population.push( new Invader(this.randomPlace() , Math.random()*-20, shape) );
		}
	}
	...
	elitism(){
		...
		let invader = new Invader(this.randomPlace() ,Math.random()*-20, this.bestOfGeneration.shape.slice());
